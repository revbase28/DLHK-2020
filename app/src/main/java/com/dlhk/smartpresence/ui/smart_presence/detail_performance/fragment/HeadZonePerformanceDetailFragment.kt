...
import kotlinx.android.synthetic.main.fragment_head_zone_performance_detail.*

class HeadZonePerformanceDetailFragment(
    val employeeId: Long,
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_head_zone_performance_detail, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        getHeadZonePerformanceDetail(employeeId)
    ...
    fun getHeadZonePerformanceDetail(id: Long){
        ...
        if(viewModel.headZonePerformanceDetail.value != null) viewModel.headZonePerformanceDetail.postValue(null)

        viewModel.getHeadZonePerformanceDetail(id)
        viewModel.headZonePerformanceDetail.observe(viewLifecycleOwner, Observer { response ->
            when(response) {
                is Resource.Success -> {
                    response.data!!.let {
                        textName.text = it.data.employeeName
                        textEmployeeNumber.text = it.data.employeeNumber
                        disciplinePercentage.text = "${it.data.diciplinePresence} %"
                        fisrtSessionPercentage.text = "${it.data.firstSession} %"
                        secondSessionPercentage.text = "${it.data.secondSession} %"
                        thirdSessionPercentage.text = "${it.data.thirdSession} %"
                        zoneCleanlinessPercentage.text = "${it.data.cleanlinessZone} %"
                        teamCompletenessPercentage.text = "${it.data.completenessTeam} %"
                        garbageDataPercentage.text = "${it.data.dataOfGarbage} %"
